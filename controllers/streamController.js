const query = require('../queries/streamQuery');

exports.getAll = async (req, res) => {
  try {
    const streams = await query.getAll();
    res.status(200).json({ message: 'Data fetched Successfully', success: true, statusCode: 200, data: streams});
  } catch (err) {
...
exports.getOne = async (req, res) => {
  try {
    const stream = await query.getById(req.params.id);
    if (!stream) return res.status(404).json({ message: 'Data not found', success: false, statusCode: 404 });
    res.status(200).json({ message: 'Data fetched Successfully', success: true, statusCode: 200, data: stream});
  } catch (err) {
    res.status(500).json({ message: 'failed to fetch Data', success: false, statusCode: 500, error: err.message });
  }
};
...
exports.create = async (req, res) => {
  try {
    const newStream = await query.create(req.body);
    res.status(201).json({  message: 'Data Created Successfully', success: true, statusCode: 201, data: newStream});
  } catch (err) {
...
exports.getByParent = async (req, res) => {
  try {
    const streams = await query.getByParent(req.params.parentId);
    if (!streams.length) {
