// const query = require('../queries/yearQuery');
const query = require('../queries/yearQuery')

exports.getAll = async (req, res) => {  
  try {    
    const years = await query.getAll();
    res.status(200).json({ message: 'Data fetched Successfully', success: true, statusCode: 200, data: years});
  } catch (err) {
...
exports.getOne = async (req, res) => {
  try {
    const year = await query.getById(req.params.id);
    if (!year) return res.status(404).json({ message: 'Data not found', success: false, statusCode: 404 });
    res.status(200).json({ message: 'Data fetched Successfully', success: true, statusCode: 200, data: year});
  } catch (err) {
    res.status(500).json({ message: 'failed to fetch Data', success: false, statusCode: 500, error: err.message });
  }
};
...
exports.create = async (req, res) => {
  try {
    const newYear = await query.create(req.body);
    res.status(201).json({  message: 'Data Created Successfully', success: true, statusCode: 201, data: newYear});
  } catch (err) {
