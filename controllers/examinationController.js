const query = require('../queries/examinationQuery');

exports.getAll = async (req, res) => {
  try {
    const examinations = await query.getAll();
    res.status(200).json({ message: 'Data fetched Successfully', success: true, statusCode: 200, data: examinations});
  } catch (err) {
...
exports.getOne = async (req, res) => {
  try {
    const exam = await query.getById(req.params.id);
    if (!exam) return res.status(404).json({ message: 'Data not found', success: false, statusCode: 404 });
    res.status(200).json({ message: 'Data fetched Successfully', success: true, statusCode: 200, data: exam});
  } catch (err) {
    res.status(500).json({ message: 'failed to fetch Data', success: false, statusCode: 500, error: err.message });
  }
};
...
exports.create = async (req, res) => {
  try {
    const newExam = await query.create(req.body);
    res.status(201).json({  message: 'Data Created Successfully', success: true, statusCode: 201, data: newExam});
  } catch (err) {
...
exports.getByParent = async (req, res) => {
  try {
    const examinations = await query.getByParent(req.params.parentId);
